...
class SalesPredictions():
    def months_query( request ):
        ...
        # VALIDATE PARAMS
        try: params = ValidateRequest(['table', 'product', 'months'], request.args)
        except: 
            return {
        ...
        # TRAIN AND PREDICT
        year_dataset, reg, score, predictionsURL = Predictions.by_year_sales(dataset)
    ...
    def cant_query(request):
        ...
        try: params = ValidateRequest(['table', 'product', 'cant'], request.args)
        except: 
            return {
            'message': 'Falto un parámetro en la petición',
            'status': 400
